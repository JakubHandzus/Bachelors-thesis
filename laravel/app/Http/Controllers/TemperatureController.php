...
class TemperatureController extends Controller
{
    ...
    public function dev_create(Request $request) {
        ...
        if ($request['temperature'] >= $sensor->max && $sensor->last_temperature < $sensor->max) {
            ...
            $mail_data = array(
                ...
                'time' => Carbon::now()->toDateTimeString(),
                'sensor_id' => $sensor->id,
                'url' => env('APP_URL','localhost').'/sensor/'.$sensor->id.'/view',
                'min_max' => 'maximum',
                'min_max_value' => $sensor->max,
                'user_name' => $user->name,
                'user_surname' => $user->surname
        ...
        // if ($sensor->temperature_time == null || $sensor->lastTemp()->time < Carbon::now()->subMinutes(10)) {
            ...
            $temp->temperature = $request['temperature'];
            $temp->sensor_id = $sensor['id'];
            $temp->time = Carbon::createFromTimestamp($request['time'])->toDateTimeString();
            $temp->save();
        // }
        ...
        return $sensor->lastTemp()->time . " " . Carbon::now()->subMinutes(10);
